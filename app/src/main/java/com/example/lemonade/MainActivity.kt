...
import com.google.android.material.snackbar.Snackbar  // widget for long press events (~snackbar) appears at the bottom of the screen
...
class MainActivity : AppCompatActivity() {
    ...
    // Default variable values for start of the app:
    ...
    private var lemonTree = LemonTree()  // instance of the class defined below
    private var lemonImage: ImageView? = null  //definition data type ImageView?; "?" is to say what happens if ImageView is null?
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState != null) {        // current state can not be null
        ...
        setViewElements()
        lemonImage!!.setOnClickListener {                // not-null assertion operator converts checks that the first drawable is set.
    ...
    private fun clickLemonImage() {
        ...
        when(lemonadeState ){
            ...
            SELECT -> {
                ...
                lemonSize = lemonTree.pick()  //calling the Lemon Tree class with its method pick()
                squeezeCount = 0
            }
        ...
        setViewElements()  // function is defined below:
    }
    ...
    private fun showSnackbar(): Boolean {
        if (lemonadeState != SQUEEZE) {  // only use if lemonadeState == SQUEEZE
            return false
        }
        val squeezeText = getString(R.string.squeeze_count, squeezeCount)  // "squeeze_count" is used to look up in strings.xml; "squeezeCount" is mutable value used to insert in string %1$d
        Snackbar.make(
